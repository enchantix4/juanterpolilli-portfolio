...
export default function HeroImage({ onSectionClick, titles, colors: colorsProp }: HeroImageProps) {
  ...
  // Store synchronized container dimensions for perfect alignment
  const [containerDimensions, setContainerDimensions] = useState<{ width: number; height: number; left: number; top: number } | null>(null)
  ...
  // Synchronize container dimensions using ResizeObserver and getBoundingClientRect
  useEffect(() => {
    if (!imageContainerRef.current || !containerRef.current || !imageAspectRatio) return

    const imageContainer = imageContainerRef.current
    const mainContainer = containerRef.current
    
    const syncContainers = () => {
      if (!imageContainer || !mainContainer) return
    ...
    // Use ResizeObserver to monitor image container size changes
    const resizeObserver = new ResizeObserver(syncContainers)
    resizeObserver.observe(imageContainer)
    resizeObserver.observe(mainContainer) // Also observe parent for position changes
  ...
  useEffect(() => {
    if (!imageAspectRatio || !imageContainerRef.current || !containerRef.current) return
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => {
        if (!editMode) {
          ...
          // Check if relatedTarget is a Node and is within our container
          if (relatedTarget && relatedTarget instanceof Node && containerRef.current && containerRef.current.contains(relatedTarget)) {
            // Mouse is moving to another element within the container, don't clear
            return
          }
          // Mouse is leaving the container entirely or relatedTarget is null/not a Node
    ...
    >
      ...
      <div style={{ 
        ...
        <div 
          ...
          style={{
            ...
            height: imageAspectRatio ? undefined : '70%',
            maxWidth: '70vw',
            maxHeight: '70vh',
        ...
        <div className="absolute inset-0" style={{ pointerEvents: 'none', zIndex: 30 }}>
          ...
          <div style={{ 
            ...
            <div 
              ...
              style={{
                position: 'relative',
                // Use exact pixel dimensions when available for perfect alignment in edit mode too
                ...(containerDimensions ? {
      ...
      {!editMode && imageAspectRatio && (
        <>
          {/* Hotspot container with matching aspect ratio - synchronized with image container */}
          <div style={{ 
            position: 'absolute',
            top: 0,
            left: 0,
            ...
            <div 
              ...
              style={{
                position: 'absolute',
                // Use exact pixel dimensions and position when available for perfect alignment
                // Match image container's exact size and position to ensure pixel-perfect alignment
                ...(containerDimensions ? {
                  width: `${containerDimensions.width}px`,
                  height: `${containerDimensions.height}px`,
              ...
              {sections.map((section) => (
                <div
                  ...
                  onMouseLeave={() => {
                    ...
                    if (activeSection === section.clickId) {
                      setActiveSection(null)
                    }
                  }}
